...
from stats_utils import expected_cost, optimal_threshold_on_source, net_benefit
import os

COST_RATIOS = [(20.0, 1.0), (5.0, 1.0)]
        ...
        for model_name, model in models.items():
            for fs in ["with_soh", "no_soh"]:
                ...
                for c_fn, c_fp in COST_RATIOS:
                    row = {"target": target_name, "model": model_name, "feature_set": fs,
                           "C_FN": c_fn, "C_FP": c_fp}
                    for method in ["raw", "platt", "iso", "temp"]:
                        thr = optimal_threshold_on_source(y_src, oof_cal[method], c_fn, c_fp)
                        row[f"thr_{method}"] = thr
                        row[f"cost_{method}"] = expected_cost(y_te, p_te[method], thr, c_fn, c_fp)
                        pred = (p_te[method] >= thr).astype(int)
                        fn = np.sum((y_te == 1) & (pred == 0))
                        fp = np.sum((y_te == 0) & (pred == 1))
                        row[f"fnr_{method}"] = fn / max(y_te.sum(), 1)
                        row[f"fpr_{method}"] = fp / max((1 - y_te).sum(), 1)
                        row[f"alert_{method}"] = pred.mean()
                    rows.append(row)
                    print(f"  {target_name} {model_name} {fs} C={c_fn:.0f}:{c_fp:.0f} "
                          f"cost raw={row['cost_raw']:.4f} platt={row['cost_platt']:.4f} "
                          f"iso={row['cost_iso']:.4f}", flush=True)
