...
\graphicspath{{../data/}{./figs/}{../figs_journal_clean/}}
...
Following the original protocol \cite{shikdar2026learning}, a discharge cycle $t$ in cell $c$ is labeled as failing within horizon $H$ if any cycle $k \in [t, t{+}H)$ meets either of two criteria: (i) $\text{SOH}_k \le 0.80$, where SOH is the ratio of current discharge capacity to the mean capacity of the first 10 cycles, or (ii) the average discharge voltage at cycle $k$ falls below 94\% of its early-life baseline. Once triggered, the label stays 1 for all later cycles. Datasets without voltage data, or where voltage sits at the cutoff as in Oxford's flat LFP plateau, fall back to the SOH criterion alone.
...
A compact GRU tests whether sequence-aware representations capture trajectories that transfer across chemistries. It ingests a sliding window of $W{=}10$ consecutive cycles per cell as a $10{\times}f$ tensor, runs one GRU layer with 8 hidden units, and maps the final hidden state through a linear layer with sigmoid activation. Adam with learning rate 0.005 and binary cross-entropy loss are used. The 8-unit width was chosen to match the small number of training cells (7 to 37); wider layers overfit severely. For cross-chemistry runs the feature set is reduced to avoid unit and availability mismatches: average current is dropped (amperes for LCO, milliamperes for LFP), as are temperature and duration. The remaining features, cycle number, average voltage, minimum voltage, and SOH when enabled, are standardized with training-set statistics.

\subsection{Calibration protocol}
Two post-hoc calibration schemes are compared under a fairness-corrected protocol. Isotonic regression uses scikit-learn's \texttt{IsotonicRegression} with out-of-bounds clipping fit on the base model's training-fold scores. Platt scaling uses \texttt{LogisticRegression} with $C{=}10^{10}$ fit on the same scores. Both calibrators share identical input scores from the same underlying model, which isolates the calibration function from any confound of retraining or data splitting.

A reproducibility note is warranted. The original study \cite{shikdar2026learning} reported Brier scores around 0.032; our re-execution of the released code yields 0.17--0.26, an eightfold gap we could not resolve from the available source. Our AUC values (0.80--0.90) fall within the published range, and our qualitative conclusions hold. The Brier gap does not affect the relative comparisons that form the core of this paper.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{Fig03a_CrossChem_With_SOH_Oxford.png}
\includegraphics[width=\columnwidth]{Fig03b_CrossChem_With_SOH_Severson.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{Fig04a_CrossChem_No_SOH_Oxford.png}
\includegraphics[width=\columnwidth]{Fig04b_CrossChem_No_SOH_Severson.png}
...
Table~\ref{tab:delong} reports DeLong $p$-values for the ALL-LCO$\to$LFP ablation at H=20. On Oxford the values range from $7.2\times10^{-51}$ (XGBoost) to $6.5\times10^{-36}$ (Random Forest); on Severson all three fall below $10^{-100}$. The test accounts for the correlation between with-SOH and without-SOH AUCs computed on the same instances, so these numbers are decisive rather than a quirk of paired sampling.

\begin{table}[t]
\caption{DeLong Test $p$-Values for the SOH Ablation (ALL-LCO $\to$ LFP, H=20). All Comparisons Are Significant at $\alpha=0.05$.}
\label{tab:delong}
\centering
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{Fig06a_XGBoost_SHAP.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{Fig06d_XGBoost_SHAP_noSOH.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig_py_c_agreement.png}
\caption{On-device C engine predictions versus Python \texttt{predict\_proba()} over all 1,028 rows. Top row: agreement scatter with the identity line. Bottom row: residual histograms (C minus Python, in units of $10^{-6}$). Maximum absolute errors are $2.4\times10^{-7}$ (XGBoost), $1.1\times10^{-6}$ (LightGBM), and $3.0\times10^{-8}$ (Random Forest).}
\label{fig:pyc}
\end{figure*}
...
\caption{Prototype wiring for the ESP32-S3 deployment. The battery discharges through the INA219 shunt into a load, the 2:1 divider senses cell voltage at ADC GPIO1, and I2C (GPIO8/9) serves the INA219 and DS3231 while OneWire (GPIO10) reads the DS18B20.}
\label{fig:circuit}
\end{figure*}
...
\caption{Measured single-row inference time per model at 240 MHz: PC (gcc -O2) versus ESP32-S3 with the binary in flash (PROGMEM) and in PSRAM. PSRAM is about 1.5\texttimes\ faster than flash.}
\label{fig:latency}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.8\columnwidth]{fig_auc_latency.png}
\caption{Accuracy against inference cost for the three deployed models. AUC is the NASA H=20 Platt score; latency is measured on the ESP32-S3 with the binary in PSRAM.}
\label{fig:auc_latency}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_memory.png}
\caption{Memory footprint: the 372 kB flat binary as a share of each tier. It fits flash (2.3\%) and PSRAM (4.5\%) but exceeds the ESP32-S3's roughly 320 kB of free internal SRAM.}
\label{fig:memory}
\end{figure}
...
The embedded deployment adds a concrete positive result to these cautions. Three ensembles totaling 900 trees and 26,336 nodes fit into 372 kB, run on a \$12 microcontroller in about 2.7 ms for all three models from PSRAM, or under 0.5 ms when a single model is kept in internal SRAM, and reproduce the Python training libraries to within $10^{-6}$ across more than a thousand validation rows. Future work should pursue learned feature representations built for chemistry invariance, bidirectional transfer, and multi-seed GRU evaluation.

\section*{Acknowledgment}
No external funding was received for this work. The authors declare no conflicts of interest.

The complete source code, data processing scripts, trained models, embedded firmware, and validation pipeline are publicly available at

{\small\url{https://github.com/touhidsiddiqueeraj-bit/Multi-Horizon-Hazard-Models-for-Battery-Failure-Prediction}}.
