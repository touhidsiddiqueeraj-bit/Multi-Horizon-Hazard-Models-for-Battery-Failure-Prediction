...
on~\cite{berecibar2016}. Research effort has accordingly concentrated
...
fuzzy logic, and machine learning, are well established~\cite{berecibar2016}.
More recently, deep learning has reshaped much of this landscape:
STL-LSTM models with spatio-temporal attention~\cite{xu2024},
multilayer perceptrons~\cite{bairwa2025}, and SHAP-assisted feature
engineering~\cite{tetik2026} are representative examples, and
comprehensive reviews of RUL prediction are also
available~\cite{jin2021,li2026,thelen2024}. On the modeling side,
LightGBM~\cite{li2026lightgbm}, GRU-based~\cite{wang2021}, hybrid
neural~\cite{rastegarpanah2024}, and convolutional~\cite{sun2025}
...
frame the problem~\cite{ibraheem2025,li2024,yao2024}. What has
...
al.~\cite{pang2026} proposed a Transformer--BiLSTM framework for
...
methods~\cite{platt1999,zadrozny2002,goldstein2020}. In the
background, SHAP-based interpretability~\cite{lundberg2017} and a
growing collection of public benchmark datasets~\cite{dosreis2021,li2025}
...
test~\cite{delong1988}, the usual tool for comparing ROC curves, which
...
dataset~\cite{severson2019} makes large-scale LFP validation possible,
...
Figure~\ref{fig:framework} overviews the workflow: LCO training cells and
...
\hline
\fontsize{8}{9}\selectfont\textbf{Dataset} & \textbf{Cells} & \textbf{Chem.} & \textbf{Cycles/Cell} & \textbf{Role}\\
\hline
\fontsize{7}{8}\selectfont
NASA 18650~\cite{saha2007} & 37 & LCO & 1--445 & Training\\ \hline
CALCE CX2~\cite{calce2023} & 7 & LCO & 775--1{,}961 & Training\\ \hline
Oxford LFP~\cite{birkl2017} & 5 & LFP & 46--78 & Transfer\\ \hline
Severson~\cite{severson2019} & 141 & LFP & 101--2{,}237 & Transfer\\ \hline
\end{tabular*}
\end{table}
...
Table~\ref{tab:within} reports fold-mean Platt-calibrated AUC per
horizon. All 32 model--horizon--dataset combinations reach a fold-mean
AUC of at least 0.85 under the clean protocol
(Figure~\ref{fig:horizons}); the GRU shows the largest fold-to-fold
...
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}|l|l|c|c|c|c|c|c|}
\hline
\fontsize{8}{9}\selectfont\textbf{Model} & \textbf{Dataset} & \textbf{H=10} & \textbf{H=20} & \textbf{H=30} & \textbf{H=50} & \textbf{Mean} & \textbf{Brier}\\
\hline
\fontsize{7}{8}\selectfont
%%BAKE:tab_within
...
\hline
\fontsize{8}{9}\selectfont\textbf{Dataset} & \textbf{Method} & \textbf{AUC} & \textbf{Brier} & \textbf{ECE} & \textbf{Slope}\\
\hline
\fontsize{7}{8}\selectfont
...
uncertain (Figure~\ref{fig:reliability}).
...
Table~\ref{tab:transfer} reports transfer at $H{=}20$ with and without
...
trees in the matched comparison. Figure~\ref{fig:collapse} summarizes
...
\hline
\fontsize{8}{9}\selectfont\textbf{Pair} & \textbf{w/ SOH} & \textbf{no SOH}\\
\hline
\fontsize{7}{8}\selectfont
%%BAKE:tab_samechem
...
(Figure~\ref{fig:horizons}), with slightly larger variation at short
...
\subsection{Operational Threshold Evaluation}
...
\hline
\fontsize{8}{9}\selectfont\textbf{Target} & \textbf{Features} & \textbf{raw} & \textbf{Platt} & \textbf{Isotonic}\\
\hline
\fontsize{7}{8}\selectfont
...
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}|l|l|c|c|l|l|}
\hline
\fontsize{8}{9}\selectfont\textbf{Target} & \textbf{Model} & \textbf{w/ SOH} & \textbf{w/o} & \textbf{$\Delta$AUC [95\% CI]} & \textbf{DeLong $p$}\\
\hline
\fontsize{7}{8}\selectfont
%%BAKE:tab_soh
