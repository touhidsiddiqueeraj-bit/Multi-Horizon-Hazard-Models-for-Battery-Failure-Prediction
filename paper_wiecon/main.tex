...
\subsection{Cross-Dataset Transfer, the SOH Shortcut, and Its Controls}
...
Three controls establish what the collapse means. First, the
distance-to-threshold rule $0.80-\mathrm{SOH}$, which involves no
fitting at all, transfers to Severson at pooled AUC 0.912 (95\% CI
[0.84, 1.00]) for the ALL-LCO source --- within the confidence range of
the tuned ensembles --- so most of the apparent transfer is the SOH
coordinate itself. Second, relabeling failure by voltage sag alone, a
criterion SOH does not define, cuts with-SOH transfer from 0.90 to 0.72
on Severson, directly measuring the label circularity: with-SOH
transfer tracks how much SOH defines the label (0.985 under an
SOH-only label, 0.896 under the combined label, 0.723 under the
voltage-only label). Third, same-chemistry cross-laboratory controls
show the pattern is not chemistry-specific: Severson$\to$Oxford (both
LFP) collapses from 0.998 with SOH to 0.428 without, and
NASA$\to$CALCE (both LCO) collapses without SOH just as the
cross-chemistry pairs do. The supported conclusion is deliberately
narrower than ``SOH is chemistry-specific'': much of the observed
cross-dataset, cross-chemistry discrimination is attributable to SOH,
partly because SOH defines the failure criterion itself.
