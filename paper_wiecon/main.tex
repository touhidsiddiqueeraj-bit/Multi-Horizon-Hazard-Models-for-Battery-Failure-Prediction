...
\begin{IEEEkeywords}
battery management systems, machine learning, probability calibration,
prognostics and health management, transfer learning, dataset shift
\end{IEEEkeywords}
...
competitive lifetime prediction. Survival analysis has recently emerged
as an alternative paradigm: Ibraheem et al.~\cite{ibraheem2025} employed
Cox proportional hazards modeling, Li et al.~\cite{li2024} proposed
early-cycle feature-based prediction, and Yao et al.~\cite{yao2024}
developed a physics-guided machine-learning framework for capacity
degradation. Cross-chemistry transfer has received little direct
attention: Pang et al.~\cite{pang2026} proposed a Transformer--BiLSTM
framework for multi-chemistry SOH estimation, though their focus was
SOH rather than failure prediction. Probability calibration is
similarly underexplored, despite established methods such as Platt
scaling~\cite{platt1999}, isotonic regression~\cite{zadrozny2002}, and
survival calibration~\cite{goldstein2020}. Model interpretability using
SHAP~\cite{lundberg2017} and public benchmark datasets~\cite{dosreis2021,li2025}
...
shortcut: the composite failure label used in multi-horizon failure
prediction is defined from SOH, and SOH is also the strongest available
input feature, so a model can score well by detecting proximity to the
label threshold itself. The DeLong test~\cite{delong1988} is common for
...
Oxford release stores discharge summaries at roughly 100-cycle
intervals, so after preprocessing only 46--78 usable cycles per cell
remain and every horizon $H \in \{10,20,30,50\}$ is shorter than the
sampling interval; Oxford failure labels are therefore identical across
all four horizons, and Oxford scores measure pooled failure detection
rather than horizon-resolved prediction (per-cell statistics
effectively cover four of the five cells, one cell having too few
positive labels for a within-cell AUC to be defined). Second, because
...
window of $w=10$ cycles. In the CALCE dataset, average temperature and
...
$P_{10}\le P_{20}\le P_{30}\le P_{50}$ is enforced --- we measured such
violations in up to 46\% of within-dataset rows, which is why we use
the term \emph{failure-risk classification} rather than hazard
modeling. Second, the label is \emph{partially defined by SOH itself},
and SOH is also an input feature in Eq.~(\ref{eq:features}); a with-SOH
model can therefore detect proximity to the SOH $=0.80$ label threshold
without learning degradation dynamics. All with-SOH results below are
upper bounds that include this label-proximity leakage, and
Section~III-C quantifies it by relabeling failure with a single
endpoint at a time.

\subsection{Models and Training}
We evaluate four classifiers: XGBoost (max\_depth 4, learning rate
0.05, subsample 0.8, colsample 0.8, min\_child\_weight 5), LightGBM
with matched hyperparameters (min\_child\_samples 20), Random Forest
(300 trees, max\_depth 6), and a single-layer GRU (8 hidden units,
10-cycle input window, BCEWithLogits loss, Adam at learning rate 0.005,
early stopping with patience 10, up to 50 epochs). All tree ensembles
use 300 estimators to keep the comparison controlled. Within-dataset
evaluation uses 5-fold cell-grouped cross-validation on NASA (37
cells) and leave-one-cell-out on CALCE (7 cells): every cycle from a
given cell stays entirely on one side of every split, models are
retrained per horizon, and the cell --- not the cycle --- is treated as
the experimental unit.

\subsection{Probability Calibration}
Raw predictions are calibrated with Platt scaling~\cite{platt1999} and
isotonic regression~\cite{zadrozny2002}, with temperature scaling as a
reference. The protocol is leakage-clean: for every evaluation fold, an
inner cell-disjoint split of the training cells produces
out-of-fold scores, and the calibrators are fitted on those
out-of-fold scores only; the untouched test cells are scored by a model
trained on all training cells and calibrated with the
out-of-fold-fitted functions. No calibrator is ever fitted on the
scores it is applied to, which removes the optimistic bias of
in-sample fitting (an earlier version of this study fitted calibrators
directly on training-fold predictions; the absolute calibration numbers
reported here are accordingly more conservative). Calibration quality
...
\begin{table}[!t]
\caption{Within-Dataset Platt-Calibrated Fold-Mean AUC and Mean Brier
Score per Horizon (Cell-Disjoint CV; CALCE Is Leave-One-Cell-Out)}
\label{tab:within}
\centering
\small
...
\caption{Within-dataset fold-mean Platt AUC across prediction horizons
(NASA and CALCE, cell-disjoint CV).}
\label{fig:horizons}
\end{figure}

\begin{table}[!t]
\caption{Calibration Method Comparison (Mean Across Tree Models;
Calibrators Fitted on Cross-Fitted Out-of-Fold Scores)}
...
Table~\ref{tab:calibration} compares the three calibrators. On
discrimination, raw scores are effectively unbeatable within dataset:
a fitted calibrator is a monotone re-mapping and preserves per-fold
ranking up to the ties it introduces, so fold-mean AUC differences
between calibrators are small. Platt achieves the best calibration
error on CALCE (ECE 0.029 versus 0.049 for isotonic), but its fitted
slope on CALCE is only 0.01: under leave-one-cell-out the
out-of-fold sigmoid essentially washes confidence toward the base rate
while preserving ranking. Isotonic's step function creates large tied
groups whose cost is invisible to ROC-AUC but severe for
precision--recall. We therefore read the comparison as: Platt
\emph{preserves discrimination best while achieving calibration error
comparable to or better than isotonic} --- a materially weaker and more
defensible claim than ``Platt is the better calibrator'' --- and raw
scores remain the safest ranking signal whenever the operating context
is uncertain (Fig.~\ref{fig:reliability}).
...
Within CALCE (XGBoost, $H{=}20$). (b) ALL-LCO$\to$Severson without SOH
($H{=}20$): no source-fitted calibrator lands near the diagonal on the
shifted target.}
\label{fig:reliability}
\end{figure}

\subsection{Cross-Dataset Transfer and the SOH Shortcut}
Table~\ref{tab:transfer} compares transfer at $H{=}20$ with and without
SOH as a feature, and Fig.~\ref{fig:collapse} summarizes the drop.
Because calibration parameters are dataset-specific, transfer is
reported using raw AUC. With SOH included, the tree ensembles reach
pooled AUC between 0.85 and 1.00 (up to 1.00 on Oxford for
NASA-trained LightGBM and Random Forest). Removing SOH collapses every
tree model: on NASA+CALCE$\to$Oxford, XGBoost falls from 0.917 to
0.429, and no no-SOH configuration exceeds 0.58 on Oxford or 0.77 on
Severson, with several configurations below chance --- the remaining
sensors do not merely lose signal, they rank the target distribution
backwards. The GRU is highly unstable across training seeds under
shift (per-cell AUC from 0.21 to 1.00 at fixed settings) and offers no
transfer advantage even though it sees the same common feature set as
the trees in the matched comparison.

\begin{table}[!t]
\caption{Cross-Dataset Transfer at $H{=}20$: Pooled Raw AUC With and
Without SOH (GRU Rows Use the Common Feature Set and Average Three
Seeds)}
\label{tab:transfer}
\centering
...
\caption{Transferability collapse map at $H{=}20$: AUC drop from
removing SOH, per source$\to$target pair and model.}
\label{fig:collapse}
\end{figure}
...
most of the apparent transfer is the SOH coordinate itself. The
...
\begin{table}[!t]
\caption{Failure-Definition Ablation at $H{=}20$ (XGBoost, ALL-LCO
Source): Pooled Raw AUC With and Without SOH as a Feature}
\label{tab:faildef}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
...
\midrule
\input{generated/tab_faildef}
\end{tabular}}
\end{table}

\begin{table}[!t]
\caption{Same-Chemistry Cross-Laboratory Controls at $H{=}20$
(XGBoost): Pooled Raw AUC With and Without SOH}
\label{tab:samechem}
\centering
\small
\begin{tabular}{lcc}
...
\begin{table}[!t]
\caption{Fixed-Horizon XGBoost Versus the Discrete-Time Hazard Model
(Survival Product), Fold-Mean AUC Within Dataset and Pooled Raw AUC on
Transfer at $H{=}20$ (Both With SOH)}
\label{tab:hazard}
\centering
\small
...
We therefore also fit an explicit discrete-time hazard model: one pooled
classifier predicts per-offset hazards $h_\ell(t) = P(T_f = t+\ell
\mid T_f \ge t+\ell,\, x_t)$ with the offset $\ell$ as an input
feature, using only the features measured at $t$ (operational), and
cumulative risk follows the survival product $R(t,H) = 1-\prod_{\ell
\le H}(1-\hat h_\ell(t))$, which is monotone in $H$ by construction
and yields all horizons from a single fit
(Table~\ref{tab:hazard}). Within datasets the survival product matches
...
--- the operationally honest choice, since no target labels exist at
deployment --- and apply it unchanged to the target
(Table~\ref{tab:operational}). On Severson with SOH, the transferred
threshold misses 32\% of target failures while holding false alarms to
4\%; without SOH the miss rate more than doubles to 70\% with the full
budget spent, and calibrated scores differ only marginally because
under shift the ranking is the asset and the ranking is what collapsed.
On Oxford, nearly every row is positive under the coarse pooled labels,
so virtually any threshold alerts on almost every cycle --- that target
offers no room for a discriminating operating point at all
(Section~II-B).

\begin{table}[!t]
\caption{Operational Evaluation at $H{=}20$ (XGBoost, ALL-LCO Source):
Threshold Selected on Source Out-of-Fold Scores With a 10\%
False-Alarm Budget, Then Applied Unchanged to the Target --- Target
FNR\% / FPR\%}
...
\midrule
\input{generated/tab_operational}
\end{tabular}}
\end{table}

\begin{table}[!t]
\caption{SOH Ablation at $H{=}20$ (ALL-LCO Source): Pooled AUC With and
Without SOH, Paired Cell-Level Bootstrap 95\% CI on the Difference
(Primary), and Cycle-Level DeLong $p$-Value (Secondary; Inflated by
...
Several limitations bound these claims. The fixed-horizon formulation
consists of independent classifiers with no enforced monotonicity
across horizons; a discrete-time survival formulation that restores
monotonicity by construction is a natural next step. The Oxford
dataset's coarse 100-cycle sampling makes its labels horizon-invariant
and limits it to pooled-detection use; CALCE contributes only seven
cells. Calibration was analyzed with cross-fitted out-of-fold fits but
remains in-sample for the GRU transfer setting, and decision-threshold
analysis under shift was left to the extended version of this study.
Cross-chemistry evaluation covered LCO and LFP only, and all datasets
are laboratory-scale. Future work will extend the framework to
minimal-feature baselines as standard practice, additional chemistries
and controlled same-laboratory cross-chemistry campaigns, continuous
time-to-failure modeling, cell-level hierarchical calibration, and
