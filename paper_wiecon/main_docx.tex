...
including STL-LSTM with spatio-temporal attention~[2],
multilayer perceptrons~[3], and SHAP-assisted feature
engineering~[4]. Extensive reviews of RUL prediction are
available in~[5]--[7], while LightGBM~[8],
GRU-based~[9], hybrid neural~[10], and
convolutional architectures~[11] have demonstrated
competitive lifetime prediction. Survival analysis has recently emerged as an alternative
paradigm~[12]--[14]. Cross-chemistry transfer
has received little direct attention: Pang et al.~[15]
...
methods~[16]--[18]. Model
interpretability using SHAP~[19] and public benchmark
datasets~[20], [21] continue to advance the field.
...
Severson dataset~[23] enables large-scale LFP
...
datasets, features and labels, model families, per-horizon classifiers,
cross-fitted calibration, and the reliability, transfer, and
shortcut-control evaluations.}
\label{fig:framework}
\end{figure}
...
Table II shows fold-mean Platt-calibrated AUC per
...
Within CALCE (XGBoost, $H{=}20$; ECE: raw 0.11, Platt 0.04, isotonic
0.05). (b) ALL-LCO$\to$Severson without SOH ($H{=}20$): no
source-fitted calibrator lands near the diagonal on the shifted
target.}
\label{fig:reliability}
\end{figure}

\subsection{III-C Cross-Dataset Transfer and the SOH Shortcut}
Table IV compares transfer at $H{=}20$ with and
