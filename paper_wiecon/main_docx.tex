...
\title{When Battery Failure Models Appear to Transfer: Exposing Health-State Shortcuts Under Dataset and Chemistry Shift}
...
\noindent\textbf{\textit{Keywords---}}\textit{battery management
systems, machine learning, probability calibration, prognostics and
health management, transfer learning, dataset shift.}
...
treats cycle-level predictions as independent despite within-cell
correlation, which can produce anti-conservative inference and
artificially small $p$-values (pseudoreplication). We therefore use
DeLong only as a directional consistency check.
...
\begin{table}[!t]
\caption{Within-Dataset AUC and Brier Score}
\label{tab:within}
\centering
...
\begin{table}[!t]
\caption{Cross-Dataset Transfer at $H{=}20$ (Pooled Raw AUC)}
\label{tab:transfer}
\centering
\begin{tabular}{|l|l|c|c|c|c|}
\hline
...
under the voltage-sag endpoint (0.723 versus 0.611 without SOH) is
consistent with SOH retaining some genuine health-state information
even when it does not directly define the endpoint.

\begin{table}[!t]
\caption{Failure-Definition Ablation at $H{=}20$}
\label{tab:faildef}
\centering
...
\subsection{III-F Operational Threshold Evaluation}
...
\begin{table}[!t]
\caption{Operational Evaluation at $H{=}20$ (FNR\% / FPR\%)}
...
\begin{table}[!t]
\caption{SOH Ablation at $H{=}20$ (Bootstrap CI, DeLong $p$)}
\label{tab:soh}
\centering
\begin{tabular}{|l|l|c|c|l|l|}
\hline
\textbf{Target} & \textbf{Model} & \textbf{w/ SOH} & \textbf{w/o} & \textbf{$\Delta$AUC [95\% CI]} & \textbf{DeLong $p$}\\
\hline
%%BAKE:tab_soh
Oxford & XGBoost & 0.917 & 0.429 & +0.488 [NA; n=5] & $7.2e-51$ \\ \hline
Oxford & LightGBM & 0.971 & 0.332 & +0.639 [NA; n=5] & $2.6e-64$ \\ \hline
Oxford & Random Forest & 0.989 & 0.581 & +0.409 [NA; n=5] & $6.5e-36$ \\ \hline
...
so the cycle-level DeLong $p$-values, which treat correlated cycles
as independent and can therefore be anti-conservatively small under
