...
AUC treats all errors as equal; a BMS does not. Using the cost $C(\tau) = C_{FN}\,FN(\tau) + C_{FP}\,FP(\tau)$ with a 20:1 miss-to-false-alarm ratio, and selecting the decision threshold on \emph{source} out-of-fold scores --- the operationally honest choice, since no target labels exist at deployment --- Table~\ref{tab:oper} compares the score types on the shifted targets. With SOH available the scores support a usable operating point on both targets. Without SOH, no threshold reproduces the source operating point: expected cost rises @@COST_RATIO_SEV@@$\times$ over the with-SOH case on Severson, and the calibrated scores offer no advantage over raw ones --- under shift, the ranking is the asset, and the ranking is what collapsed. Fig.~\ref{fig:netbenefit} shows the corresponding decision-curve view. Calibration, in other words, is a second-order concern in this regime: first the model must rank the target cells, which --- per Sections~III-C--III-G --- only SOH lets it do.

\begin{table*}[!t]
\caption{Operational Cost at H=20 (XGBoost, ALL-LCO Source, $C_{FN}{:}C_{FP} = 20{:}1$, Threshold Selected on Source Out-of-Fold Scores). FN Rate Is the Fraction of True Failures Missed.}
\label{tab:oper}
\centering
\resizebox{\textwidth}{!}{%
...
\textbf{Target} & \textbf{Features} & \textbf{cost} & \textbf{FN} & \textbf{cost} & \textbf{FN} & \textbf{cost} & \textbf{FN} & \textbf{cost} & \textbf{FN}\\
\midrule
@@TAB_OPER@@
\bottomrule
\end{tabular}}
